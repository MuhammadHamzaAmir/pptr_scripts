...
const nexus5X = puppeteer.devices["Nexus 5X"];
...
const argv = require("yargs")
  ...
  .help()
  .example(
    "$0 --url https://tailwinduikit.com https://getfigit.com"
  )
  .example(
    "$0 --no-throttle --no-mobile -u https://tailwinduikit.com https://getfigit.com"
  )
  .example(
    "$0 -u https://www.bing.com/ https://www.google.com/ https://www.yahoo.com/"
  )
  .wrap(null).argv;

const urls = argv.url.length
  ? argv.url
  : ["https://tailwinduikit.com", "https://getfigit.com"];
...
async function launch(position, screen) {
  ...
  const totalWidthOfWindowsWithSpacers =
    totalWidthOfWindows + totalSpacerWidthAddition;
...
(async () => {
  ...
  DEFAULT_VIEWPORT.width = MOBILE
    ? nexus5X.viewport.width
    : Math.floor(screen.width / urls.length);
  ...
  const waitForPage = async (pos) => {
    ...
    const url = urls[pos];
    return page.goto(url, { waitUntil: "networkidle2" }).then(() => Date.now());
  };

  const stopTimes = await Promise.all(urls.map((url, i) => waitForPage(i)));
  stopTimes.forEach((stopTime, i) =>
    console.log(
      `Page ${i + 1} took ${stopTime - start} ms to reach network idle`
    )
  );
